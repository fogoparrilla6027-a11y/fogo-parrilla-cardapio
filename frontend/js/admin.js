...
async function loadProductsAdmin() {
  ...
  try {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap">
        <table class="data-table">
          <thead><tr><th>ID</th><th>Nome</th><th>Categoria</th><th>Preço</th><th>Destaque</th><th>Disponível</th><th>Ações</th></tr></thead>
...
async function saveCategory(id) {
  const data = {
    ...
    icon: document.getElementById('cfIcon').value,
    order: parseInt(document.getElementById('cfOrder').value)
  };
  try {
    if (id) {
    ...
    el.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
        <div class="form-group">
          ...
          <label class="toggle" style="display:block;margin-top:0.3rem;">
            <input type="checkbox" id="pixEnabled" ${config.enabled ? 'checked' : ''}>
            <span class="toggle-slider"></span>
          </label>
        </div>
        <div class="form-group">
          <label>Modo Teste (offline)</label>
          <label class="toggle" style="display:block;margin-top:0.3rem;">
            <input type="checkbox" id="pixTestMode" ${config.modo_teste ? 'checked' : ''}>
            <span class="toggle-slider"></span>
          </label>
        </div>
      ...
      <div style="margin-top:1rem;">
        <p style="font-size:0.8rem;color:var(--text-muted);margin-bottom:1rem;">
          ⚠️ O certificado .pfx está configurado em <code style="background:var(--bg);padding:0.1rem 0.3rem;border-radius:4px;">data/pix_config.json</code>.
          Modo offline gera QR Code estático (EMV) sem consultar a API do Sicoob.
        </p>
        <button class="btn btn-primary" onclick="savePixConfig()">💾 Salvar Configuração PIX</button>
        <button class="btn btn-secondary" onclick="testPix()" style="margin-left:0.5rem;">🧪 Gerar PIX Teste</button>
      </div>
    `;
  } catch (e) { el.innerHTML = '<p style="color:var(--danger);">Erro ao carregar configuração PIX</p>'; }
}

async function savePixConfig() {
  try {
    await api('/api/pix/config', {
      method: 'PUT',
      body: JSON.stringify({
        enabled: document.getElementById('pixEnabled').checked,
        modo_teste: document.getElementById('pixTestMode').checked,
        pix_key: document.getElementById('pixKey').value,
        merchant_name: document.getElementById('pixMerchant').value,
        merchant_city: document.getElementById('pixCity').value,
        client_id: document.getElementById('pixClientId').value
      })
    });
...
async function testPix() {
  try {
    const data = await api('/api/pix/cobranca', {
      method: 'POST',
      body: JSON.stringify({ valor: 0.01, orderId: null })
    });
...
async function loadStore() {
  ...
  try {
    ...
    el.innerHTML = `
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
        <div class="form-group"><label>Nome</label><input type="text" id="sName" value="${store.name}"></div>
        <div class="form-group"><label>Slogan</label><input type="text" id="sSlogan" value="${store.slogan || ''}"></div>
        <div class="form-group" style="grid-column:1/-1;"><label>Descrição</label><textarea id="sDesc" rows="2">${store.description}</textarea></div>
        <div class="form-group" style="grid-column:1/-1;"><label>Endereço</label><input type="text" id="sAddress" value="${store.address}"></div>
        <div class="form-group"><label>Telefone</label><input type="text" id="sPhone" value="${store.phone}"></div>
        <div class="form-group"><label>WhatsApp</label><input type="text" id="sWhats" value="${store.whatsapp}"></div>
        <div class="form-group"><label>Instagram</label><input type="text" id="sInsta" value="${store.instagram}"></div>
        <div class="form-group"><label>Taxa de Entrega</label><input type="number" id="sDelivery" step="0.01" value="${store.delivery_fee}"></div>
        <div class="form-group"><label>Pedido Mínimo</label><input type="number" id="sMinOrder" step="0.01" value="${store.min_order}"></div>
        <div class="form-group" style="grid-column:1/-1;"><label>Horários</label><input type="text" id="sHours" value="${store.working_hours}"></div>
      </div>
      <button class="btn btn-primary" onclick="saveStore()" style="margin-top:1rem;">💾 Salvar Loja</button>
    `;
  } catch (e) { el.innerHTML = '<p style="color:var(--danger);">Erro ao carregar loja</p>'; }
}
  ...
  try {
    await api('/api/admin/store', {
      ...
      body: JSON.stringify({
        ...
        slogan: document.getElementById('sSlogan').value,
        description: document.getElementById('sDesc').value,
        address: document.getElementById('sAddress').value,
        phone: document.getElementById('sPhone').value,
        whatsapp: document.getElementById('sWhats').value,
